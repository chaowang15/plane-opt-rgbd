...
bool RGBDMesh::readPLY(const string filename)
{
	FILE* fin;
	...
	while (true)
	{
		if (fgets(line, 1024, fin) == NULL) continue;
			...
			if (vertex_mode % 2 == 1)
			{
				if (property_num >= 3) // skip property x,y,z
				{
					...
					if (!strcmp(token, "uchar")) // color
					{
						...
						{
							cout << "ERROR in Reading PLY model: cannot read this vertex color type -- " << string(token) << endl;
							return false;
						}
					}
...
bool RGBDMesh::readMTLandTextureImages(const string filename)
{
	...
	while (!readin.eof() && !readin.fail())
	{
		...
		if (!readin.good() || readin.eof()) break;
	...
	for (int i = 0; i < texture_img_num_; ++i)
		ori_texture_images_[i].copyTo(texture_image_(cv::Rect(0, texture_image_height_ - image_y_bases_[i] - ori_texture_images_[i].rows,
...
bool RGBDMesh::readOBJ(const string filename)
{
	ifstream readin(filename, ios::in);
	if (readin.fail() || readin.eof())
	{
	...
	while (!readin.eof() && !readin.fail())
	{
		getline(readin, str_line);
		if (!readin.good() || readin.eof()) break;
		if (str_line.size() <= 1) continue;
		istringstream iss(str_line);
		iss >> str_first;
		if (str_first[0] == '#') continue;
		...
		else if (str_first == "f")
		{
			...
			while (loop-- > 0)
			{
				...
				if (flag_vtx_texture_)
				{
					...
					vtx.uv[1] = (uvs[vt][1] * ori_texture_images_[cur_tex_idx].rows + image_y_bases_[cur_tex_idx]) / (texture_image_height_);
					if (flag_vtx_normal_)
...
void RGBDMesh::initModelDataBuffer()
{
	...
	glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void*)offsetof(Vertex, color)); // name of 'color' variable in struct Vertex

	// texture uv coordinates
	if (flag_vtx_texture_)
	{
		glEnableVertexAttribArray(2);
		glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void*)offsetof(Vertex, uv)); // name of 'color' variable in struct Vertex
	}
	// normals
	if (flag_vtx_normal_)
	{
		glEnableVertexAttribArray(3);
		glVertexAttribPointer(3, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex), (void*)offsetof(Vertex, normal)); // name of 'color' variable in struct Vertex
	}
	...
	//glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, texture_image_width_, texture_image_height_, 0, GL_BGR, GL_UNSIGNED_BYTE, texture_image_.data);
...
void RGBDMesh::prepareImageBuffer()
{
	image_buffer_.bindForWriting();
}

void RGBDMesh::extractImageBuffer()
{
...
void RGBDMesh::saveColor2PNG(const string filename)
{
...
void RGBDMesh::saveDepth2PNG(const string filename)
{
	image_buffer_.setReadBuffer(GBuffer::GBUFFER_TEXTURE_TYPE_DEPTH);
	glReadPixels(0, 0, kImageWidth, kImageHeight, GL_RGB, GL_FLOAT, image_buffer_arr_);
...
void RGBDMesh::saveVisibilityImage2Binary(const string filename)
{
	image_buffer_.setReadBuffer(GBuffer::GBUFFER_TEXTURE_TYPE_DEPTH);
	glReadPixels(0, 0, kImageWidth, kImageHeight, GL_RGB, GL_FLOAT, image_buffer_arr_);
...
void RGBDMesh::saveVisibleVertices2Binary(const string filename)
{
	image_buffer_.setReadBuffer(GBuffer::GBUFFER_TEXTURE_TYPE_DEPTH);
	glReadPixels(0, 0, kImageWidth, kImageHeight, GL_RGB, GL_FLOAT, image_buffer_arr_);
...
glm::mat4 RGBDMesh::computeTransformation()
{
	...
	glm::mat4 View = glm::lookAt(
		glm::vec3(0, 5, 1), // Camera is at (4,3,3), in World Space
...
glm::mat4 RGBDMesh::computeTransformationForFrame(int frame_idx)
{
	...
	glm::mat4 trans_camera = glm::lookAt(
...
void RGBDMesh::computePerspectiveMatrix()
{
...
void RGBDMesh::readCameraPoses(const string filepath, int start_fidx, int end_fidx)
{
...
void RGBDMesh::readCameraIntrinsicsFile(const string filename)
{
